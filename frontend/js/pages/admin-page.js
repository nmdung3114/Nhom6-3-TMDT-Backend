...
// ── Daily Revenue & Order Details Search ──────────────────
function renderDailySearchUI() {
  const chartCanvas = document.getElementById('revenue-chart');
  if (!chartCanvas) return;

  const container = chartCanvas.closest('.admin-grid') || chartCanvas.parentElement.parentElement;
  ...
  searchCard.innerHTML = `
    <div class="card__header" style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:15px">
      <h3 style="margin:0">📊 Chi tiết doanh thu theo ngày</h3>
      <div style="display:flex; gap:10px; align-items:center">
        <input type="date" id="daily-search-date" class="form-control" style="width:180px; margin:0">
    ...
    <div id="daily-search-result" style="padding:20px; display:none">
      <div id="daily-summary-cards" style="display:grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap:20px; margin-bottom:24px"></div>
      <div class="table-responsive">
        <table class="table">
          ...
          <tbody id="daily-orders-tbody"></tbody>
        </table>
      </div>
    </div>
  `;
  ...
  document.getElementById('daily-search-date').value = new Date().toISOString().split('T')[0];
}

window.searchDailyRevenue = async () => {
  const date = document.getElementById('daily-search-date').value;
  ...
  try {
    // Gọi API lấy đơn hàng (Lưu ý: Backend cần hỗ trợ filter date hoặc frontend tự lọc từ list 100 đơn gần nhất)
    const data = await api.get(`/admin/orders?page_size=100&date=${date}`, true);
    ...
    summaryDiv.innerHTML = `
      ...
      <div style="padding:15px; background:var(--color-bg-tertiary); border-radius:10px; border-left:4px solid var(--color-success)">
        <div style="font-size:0.8rem; color:var(--color-text-muted)">Đơn hoàn tất</div>
        <div style="font-size:1.4rem; font-weight:700; color:var(--color-success)">${paidOrders.length} đơn</div>
      </div>
    `;
    
    if (orders.length === 0) {
      tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;padding:30px;color:var(--color-text-muted)">Không có đơn hàng nào trong ngày này.</td></tr>';
      return;
    }
    ...
    tbody.innerHTML = orders.map(o => `
      <tr>
        <td>#${o.order_id}</td>
        <td><div style="font-weight:600">${o.user_name}</div><div style="font-size:0.75rem;color:var(--color-text-muted)">${o.user_email}</div></td>
        <td style="max-width:200px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap" title="${o.items.map(i => i.product_name).join(', ')}">${o.items.map(i => i.product_name).join(', ')}</td>
        <td style="font-weight:700; color:var(--color-accent-light)">${formatPrice(o.total_amount)}</td>
        <td><span class="badge ${o.status === 'paid' ? 'badge-success' : 'badge-warning'}">${o.status}</span></td>
        <td><button class="btn btn-icon btn-sm" onclick="showOrderDetails(${o.order_id})">👁️</button></td>
      </tr>`).join('');
    
    // Lưu cache để hàm showOrderDetails có thể truy xuất dữ liệu
    window._currentOrders = orders;
    
  } catch(e) {
    tbody.innerHTML = `<tr><td colspan="6" style="text-align:center;padding:30px;color:var(--color-error)">Lỗi: ${e.message}</td></tr>`;
  }
};
...
async function loadOrders(page = 1) {
  ...
  const q = new URLSearchParams({ page, page_size: 20, status, search, date });
...
window.resetOrderFilters = () => {
  ...
  const d = document.getElementById('order-date-filter');
  const st = document.getElementById('order-status-filter');
  if (s) s.value = '';
  if (d) d.value = '';
  if (st) st.value = '';
  loadOrders(1);
...
// ── Init based on page ─────────────────────────────────────
if (PAGE === 'dashboard') { loadStats(); renderDailySearchUI(); loadCharts(); }
...
if (PAGE === 'orders')    { 
  loadOrders(); 
  setupSearchFilter('order-search', loadOrders);
  document.getElementById('order-date-filter')?.addEventListener('change', () => loadOrders(1));
}
